...
module.exports.run = async function ({ api, event, args }) {
  ...
  try {
    const apiUrl = `https://vneerapi.onrender.com/chatgpt?q=${encodeURIComponent(userMessage)}`;
...
module.exports.handleReply = async function ({ api, event, handleReply }) {
  ...
  try {
    const apiUrl = `https://vneerapi.onrender.com/chatgpt?q=${encodeURIComponent(body)}`;
    const response = await axios.get(apiUrl);
    const responseMessage = response.data.message || "Sorry, I couldn't understand that.";
