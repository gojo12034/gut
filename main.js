...
const fs = require("fs");
//const login = require("nexus-fca/index.js");
//const login = require("chatbox-fca-remake/index.js")
const login = require('@dongdev/fca-unofficial/index.js');
...
try {
 ...
 // return;
  ...
  // Check if appState is null or invalid
  ...
  login(loginData, (err, api) => {
    if (err) {
      // Handle error and retry login with email/password
      if (!appState && config.email && config.password) {
        console.warn("Initial login failed. Retrying with email and password...");
        return login(
          ...
          (retryErr, retryApi) => {
            ...
            let d = JSON.stringify(appState, null, "\x09");
